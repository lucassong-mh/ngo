...
/// A Wrapper to handle the sync INode and async AsyncInode
...
impl InodeHandle {
    ...
    pub async fn lookup_no_follow(&self, path: &str) -> Result<Self> {
        self.lookup(path, 0).await
    }

    pub async fn lookup(&self, path: &str, max_follow_times: usize) -> Result<Self> {
        match self {
            Self::Sync(i) => {
                let inode = i.lookup_follow(path, max_follow_times)?;
                Ok(Self::from_sync(inode))
            }
            Self::Async(i) => {
                let inode = i.lookup_follow(path, max_follow_times).await?;
                Ok(Self::from_async(inode))
            }
        }
    }
